...
h.onKeyDown);i=a("body").css("overflow");a("body").css("overflow","hidden");h.doLayout();h.isOpen=!0;var b=a(window).width(),d=a(window).height(),l=null;a(window).bind("resize",function(){var c=a(window).width(),e=a(window).height();if((b!=c||d!=e)&&l==null)clearTimeout(l),l=setTimeout(function(){h.doLayout();b=a(window).width();d=a(window).height();l=null},10)})}},close:function(){Muse.Utils.refreshIframesAndObjectsToPauseMedia(a(".Container",g));f.unbind("click",h.close);a(window).unbind("resize",
h.doLayout);a(document).unbind("keydown",h.onKeyDown);if(c.onClose)c.onClose();f.css({opacity:0.99}).stop(!0);g.css({opacity:0.99}).stop(!0);g.animate({opacity:0},{queue:!1,duration:c.duration,complete:function(){f.animate({opacity:0},{queue:!1,duration:c.duration,complete:function(){e.hide();g.css({opacity:""});f.css({opacity:""})}})}});a("body").css("overflow",i);h.isOpen=!1},onKeyDown:function(a){a.keyCode===27&&h.close()},doLayout:function(){e.css({width:0,height:0});c.$overlayWedge.css({width:0,
height:0});e.css({width:a(document).width()+5,height:a(document).height()});c.$overlayWedge.css({width:a(document).width()-c.overlayExtraWidth+5,height:a(document).height()-c.overlayExtraHeight})}};g.data("museOverlay",h);c.autoShow&&h.open()})}})(jQuery);
